...
function getfirstPagePkmn() {
    ...
    if ($select->execute()) {
        ...
        foreach ($results as $pkmn) {
            ...
            $typeImage = explode(', ', $pkmn['type_images']);
    ?>
            <article class="card-pkmn <?= lcfirst($type[0]) ?>">
                ...
                <div class="data-pkmn <?= lcfirst($type[0]) ?>">
                    ...
                    <blockquote><?= $cleaned_text = clean_encode_and_escape_text(strtolower($pkmn['description'])) ?></blockquote>
